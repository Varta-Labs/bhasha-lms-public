<template>
	<Disclosure v-slot="{ open }" :key="chapter.name" :defaultOpen="defaultOpen">
		<DisclosureButton class="flex items-center w-full px-3 py-2.5 rounded-xl transition-all duration-200 hover:bg-purple-50/60 dark:hover:bg-purple-900/20 group">
			<span
				:class="{
					'rotate-90': open,
					'rtl:rotate-180': !open,
				...
				class="lucide-chevron-right size-4.5 text-bhasha-purple dark:text-purple-400 stroke-2 transform duration-200 shrink-0"
			/>
			<div
				class="ms-2.5 min-w-0 flex-1 text-start"
				:class="inlineSelect ? '' : 'flex items-baseline justify-between gap-3'"
				@click="redirectToChapter"
			>
				<div
					class="truncate text-base font-semibold leading-5 text-ink-gray-9 group-hover:text-bhasha-purple transition-colors"
					:title="chapter.title"
				>
					{{ chapter.title }}
				</div>
				<div
					...
					class="text-ink-gray-5 shrink-0 font-medium"
					:class="inlineSelect ? 'mt-0.5 text-xs leading-4' : 'text-xs px-2.5 py-0.5 rounded-full bg-surface-gray-2 dark:bg-ink-gray-7/40'"
				>
			...
			<span
				...
				class="lucide-check size-4.5 text-emerald-600 dark:text-emerald-400 stroke-2"
			/>
		</DisclosureButton>
		<DisclosurePanel v-if="!chapter.is_scorm_package">
			...
			>
				<template #item="{ element: lesson }">
					<div
						class="outline-lesson rounded-lg ms-5 my-1 ps-4 py-2 pe-3 text-ink-gray-9 transition-colors hover:bg-purple-50/50 dark:hover:bg-purple-900/20"
						:class="isActiveLesson(lesson.number) ? 'bg-purple-50/80 dark:bg-purple-900/30 text-bhasha-purple font-semibold' : ''"
					>
						<component
							:is="inlineSelect || disableNavigation ? 'div' : 'router-link'"
							:to="inlineSelect || disableNavigation ? undefined : lessonRoute(lesson)"
							:class="inlineSelect && !disableNavigation ? 'cursor-pointer' : ''"
							@click="onLessonClick(lesson)"
						>
							<div class="flex items-center text-sm leading-5 group">
								<MonitorPlay
									v-if="lesson.icon === 'icon-youtube'"
									class="size-4.5 stroke-2 shrink-0 me-2.5 text-purple-600 dark:text-purple-400"
								/>
								<HelpCircle
									v-else-if="lesson.icon === 'icon-quiz'"
									class="size-4.5 stroke-2 shrink-0 me-2.5 text-amber-500 dark:text-amber-400"
								/>
								<NotebookPen
									v-else-if="lesson.icon === 'icon-assignment'"
									class="size-4.5 stroke-2 shrink-0 me-2.5 text-blue-600 dark:text-blue-400"
								/>
								<SquareCode
									v-else-if="lesson.icon === 'icon-code'"
									class="size-4.5 stroke-2 shrink-0 me-2.5 text-emerald-600 dark:text-emerald-400"
								/>
								<FileText
									v-else-if="lesson.icon === 'icon-list'"
									class="size-4.5 stroke-2 shrink-0 me-2.5 text-indigo-600 dark:text-indigo-400"
								/>
								{{ lesson.title }}
								...
								<Check
									...
									class="size-4.5 stroke-2 text-emerald-600 dark:text-emerald-400 ms-2 shrink-0"
								/>
							</div>
						</component>
					</div>
...
function onLessonClick(lesson: OutlineLesson) {
	if (props.disableNavigation || !props.inlineSelect) return
	emit('select-lesson', {
