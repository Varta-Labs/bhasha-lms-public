<template>
	<div class="lesson-builder">
		<header class="lesson-builder__header">
			<div>
				<p>{{ __('Lesson builder') }}</p>
				<h2>{{ lesson.title || __('Untitled lesson') }}</h2>
			</div>
			<span>{{ __('Changes autosave while you work') }}</span>
		</header>
		<div class="lesson-builder__section is-notes">
			<div class="lesson-builder__inner">
				<div
					class="lesson-builder__section-toggle flex justify-between cursor-pointer"
					@click="
						() => {
		...
		<div class="lesson-builder__section">
			<div class="lesson-builder__inner">
				<label class="lesson-builder__label block font-medium mb-1">
					{{ __('Content') }}
				</label>
				<div
					id="content"
...
.lesson-builder__header {
	display: flex;
	align-items: flex-end;
	justify-content: space-between;
	gap: 1rem;
	padding: 1.5rem clamp(1.25rem, 7vw, 3.5rem);
	border-bottom: 1px solid #eeeaf1;
	background: linear-gradient(135deg, rgba(108, 92, 231, 0.065), transparent);
}

.lesson-builder__header p {
	color: #4a38c2;
	font-size: 0.7rem;
	font-weight: 800;
...
.lesson-builder__header > span {
	color: #7c7c7c;
	font-size: 0.7rem;
	font-weight: 600;
}

.lesson-builder__section {
	padding: 1.5rem 0 0;
	border-top: 1px solid #eeeaf1;
}

.lesson-builder__section.is-notes {
	border-top: 0;
}

.lesson-builder__inner {
	width: min(86%, 56rem);
	margin-inline: auto;
}

.lesson-builder__section-toggle,
.lesson-builder__label {
	color: #514b59;
	font-size: 0.8125rem;
	font-weight: 700;
}

.lesson-builder :deep(.ce-block__content),
.lesson-builder :deep(.ce-toolbar__content) {
	max-width: none;
}
