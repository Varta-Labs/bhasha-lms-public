<template>
	<div
		class="course-editor grid grid-cols-1 md:grid-cols-[70%,30%] flex-1 min-h-0"
	>
		<div class="course-editor__canvas flex flex-col overflow-hidden">
			<div class="overflow-y-auto h-full">
				<SkeletonLoader
					v-if="outline.loading && !outline.data"
					variant="editor-content"
				/>
				<div v-else-if="!selected" class="course-editor__empty">
					<div class="course-editor__empty-icon">
						<BookOpen class="size-7 stroke-[1.7]" />
					</div>
					<strong>{{ __('Choose a lesson to begin') }}</strong>
					<span>{{
						__(
							'Select a lesson from the outline to edit its content or preview the learner experience.',
						)
					}}</span>
				</div>
				<LessonForm
		...
		<aside class="course-editor__sidebar overflow-y-auto">
			<SkeletonLoader
				v-if="outline.loading && !outline.data"
...
.course-editor__canvas {
	padding: 1.25rem;
}

.course-editor__canvas > div {
...
.course-editor__empty strong {
	color: #383838;
	font-family: var(--bhasha-font-display);
	font-size: 1.05rem;
}

.course-editor__empty span {
	max-width: 28rem;
	font-size: 0.8125rem;
	line-height: 1.55;
}
...
.course-editor__sidebar {
	...
	background:
		linear-gradient(180deg, rgba(108, 92, 231, 0.055), transparent 9rem), #fff;
}
