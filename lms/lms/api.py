...
def get_branding():
	...
	# Website Settings may retain framework defaults after installation. Always
	# expose the public Bhasha identity to the SPA.
	settings.app_name = "Bhasha"
	settings.app_logo = {"file_url": "/assets/lms/frontend/bhasha-logo-text-transparent.png"}
...
def get_pwa_manifest():
	title = "Bhasha"
	lms_route = get_lms_route().rstrip("/")

	manifest = {
		"id": lms_route,
		"name": title,
		"short_name": title,
		"description": "Learn Indian languages with Bhasha.",
		"start_url": f"{lms_route}/",
		"scope": f"{lms_route}/",
		"icons": [
			{
				"src": "/assets/lms/frontend/bhasha-logo-4-social.png",
				"sizes": "192x192",
				"type": "image/png",
...
def clear_demo_data():
	frappe.only_for("Moderator")
	for quiz_title in ("Do you know Frappe Learning?", "Do you know Bhasha?"):
		if frappe.db.exists("LMS Quiz", {"title": quiz_title}):
			frappe.db.delete("LMS Quiz", {"title": quiz_title})

	demo_course = frappe.get_all(
		"LMS Course", {"title": ["in", ("A guide to Frappe Learning", "A guide to Bhasha")]}, pluck="name"
	)
