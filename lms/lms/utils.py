...
def get_course_details(course: str):
	...
	for json_field in ("course_highlights", "learning_outcomes", "faqs", "testimonials"):
		raw_val = course_details.get(f"{json_field}_json")
		parsed_val = []
		if raw_val:
			try:
				...
				parsed_val = []
		course_details[json_field] = parsed_val
...
def update_payment_record(doctype: str, docname: str):
	payment_name = frappe.flags.data.get("payment") if getattr(frappe.flags, "data", None) else None
	request = get_integration_requests(doctype, docname, payment_name)
...
def get_integration_requests(doctype: str, docname: str, payment_name: str = None):
	...
	return [request for request in requests if frappe._dict(json.loads(request.data)).get("payment") == payment_name]
...
def is_demo_course(course: str) -> bool:
	...
	return title in {"A guide to Frappe Learning", "A guide to Bhasha"}
