...
def get_context():
	...
	# Keep the first server-rendered frame branded as Bhasha before the SPA loads.
	favicon = BHASHA_FAVICON
	title = BHASHA_NAME
...
def get_meta_from_document(app_path):
	...
	if re.match(r"^courses/.*$", app_path):
		...
		desc = course.short_introduction or course.description or ""
		if desc:
			soup = BeautifulSoup(desc, "html.parser")
			desc = escape_html(soup.get_text())[:300]

		course_link = get_lms_route(f"courses/{course_name}")

		# Build JSON-LD Course & FAQ Schema
		schema_list = []
		course_schema = {
			...
			"provider": {
				"@type": "Organization",
				"name": "Bhasha Learning",
				"sameAs": frappe.utils.get_url(),
			},
		}
		if flt(course.rating) > 0:
			course_schema["aggregateRating"] = {
				"@type": "AggregateRating",
				"ratingValue": str(flt(course.rating)),
				"ratingCount": str(cint(course.enrollments) or 1),
			}
		if course.paid_course and course.course_price:
			course_schema["offers"] = {
				"@type": "Offer",
				"price": str(flt(course.course_price)),
				"priceCurrency": course.currency or "INR",
				"category": "Paid",
			}
		else:
			course_schema["offers"] = {
		...
		schema_list.append(course_schema)

		if course.faqs_json:
			try:
				faqs = json.loads(course.faqs_json)
						...
						"mainEntity": [
							{
								...
								"acceptedAnswer": {
									"@type": "Answer",
									"text": escape_html(BeautifulSoup(f.get("answer", ""), "html.parser").get_text()),
								},
							}
							for f in faqs
		...
		schema_json = json.dumps(schema_list if len(schema_list) > 1 else schema_list[0])
